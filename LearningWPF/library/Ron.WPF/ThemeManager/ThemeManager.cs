        ...
        public static IEnumerable<Accent> Accents
        {
            get
            {
                ...
                try
                {
                    ...
                    {
                        var resourceAddress = new Uri(string.Format("pack://application:,,,/Ron.WPF;component/Styles/Accents/{0}.xaml", color));
                        _accents.Add(new Accent(color, resourceAddress));
                    }
                }
                catch (Exception e)
                {
                    throw new AppsException("This exception happens because you are maybe running that code out of the scope of a WPF application. Most likely because you are testing your configuration inside a unit test.", e);
                }
